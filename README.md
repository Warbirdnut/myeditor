...
def replace_text():
    """
    This function allows the user to enter the word to replace and the replacement word.
    It then highlights occurrences of the word to be replaced, prompts the user for action
    (replace, skip, or cancel), and iterates until all replacements are done or canceled.
    """

    global text_widget

    def get_replacement_info():
        """
        Prompts the user for the word to replace and the replacement word.
        Returns a tuple containing (word_to_replace, replacement_word).
        """
        replace_window = tk.Toplevel(text_widget)
        replace_window.title("Replace Text")

        label1 = tk.Label(replace_window, text="Find:")
        label1.pack()

        word_to_replace_entry = tk.Entry(replace_window)
        word_to_replace_entry.pack()

        label2 = tk.Label(replace_window, text="Replace with:")
        label2.pack()

        replacement_word_entry = tk.Entry(replace_window)
        replacement_word_entry.pack()

        def get_info_and_close():
            word_to_replace = word_to_replace_entry.get()
            replacement_word = replacement_word_entry.get()
            replace_window.destroy()
            return word_to_replace, replacement_word

        replace_button = tk.Button(replace_window, text="Replace", command=get_info_and_close)
        replace_button.pack()

        replace_window.mainloop()
        return word_to_replace, replacement_word

    def highlight_word(word_to_replace):
        """
        Highlights all occurrences of the word to be replaced in the text widget.
        """
        start_index = text_widget.search(word_to_replace, "1.0", tk.END)
        while start_index:
            end_index = f"{start_index} + {len(word_to_replace)}c"
            text_widget.tag_add("highlight", start_index, end_index)
            text_widget.tag_config("highlight", foreground="yellow")
            start_index = text_widget.search(word_to_replace, end_index, tk.END)

    def replace_or_skip(word_to_replace, replacement_word):
        """
        Prompts the user to replace, skip, or cancel the replacement process.
        Returns True to continue, False to cancel.
        """
        replace_choice = tk.messagebox.askquestion(
            "Replace", f"Replace '{word_to_replace}' with '{replacement_word}'?"
        )
        if replace_choice == "yes":
            text_widget.delete("highlight", "1.0", tk.END)  # Remove previous highlighting
            text_widget.replace("highlight.first", "highlight.last", replacement_word)
            return True
        elif replace_choice == "no":
            text_widget.delete("highlight", "1.0", tk.END)  # Remove highlighting even on skip
            return True
        else:
            return False

    word_to_replace, replacement_word = get_replacement_info()
    if not word_to_replace or not replacement_word:
        return  # Do nothing if user cancels input

    highlight_word(word_to_replace)

    while True:
        replace_or_cancel = replace_or_skip(word_to_replace, replacement_word)
        if not replace_or_cancel:
            break  # Exit loop if user cancels

    text_widget.tag_delete("highlight", "1.0", tk.END)  # Remove final highlighting
